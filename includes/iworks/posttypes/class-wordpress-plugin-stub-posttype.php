...
require_once dirname( ( dirname( __FILE__ ) ) ) . '/class-mainwp-sites-report-base.php';

abstract class iworks_mainwp_sites_report_posttype_base extends iworks_mainwp_sites_report_base {
	...
	public function __construct() {
		...
		$this->posttypes_names  = apply_filters(
			'iworks/mainwp-sites-report/posttypes_names/array',
			$this->posttypes_names
		);
		$this->taxonomies_names = apply_filters(
			'iworks/mainwp-sites-report/taxonomies_names/array',
	...
	protected function get_select_array( $post_type, $atts = array() ) {
		...
		$list[0]   = __( '&mdash; Select &mdash;', 'mainwp-sites-report' );
	...
	private function render_meta_image( $post, $one ) {
		...
		$src = '';
		echo '<div class="iworks-mainwp-sites-report-image">';
		printf(
		...
		printf(
			...
			esc_attr__( 'Select Image', 'mainwp-sites-report' ),
		);
		printf(
			'<input type="button" class="button button-delete" value="%s" %s/>',
			esc_attr__( 'Delete image', 'mainwp-sites-report' ),
	...
	private function render_meta_select( $post, $one ) {
		...
		printf( '<option value="">%s</option>', __( '&mdash; Select &mdash;', 'mainwp-sites-report' ) );
	...
	private function setup_post_one_field( $post_id, $field, $meta_box_id ) {
		...
		return apply_filters(
			'iworks/mainwp-sites-report/post/meta/field',
			$field,
			$post_id
		);
	}
	...
	public function filter_add_menu_order_column( $columns ) {
		$columns['menu_order'] = __( 'Order', 'mainwp-sites-report' );
		return $columns;
	}
	...
	protected function get_taxonomy( $taxonomy_name ) {
		...
		return new WP_Error( 'taxonomy', esc_html__( 'Selected Taxonomy dosn\'t exists.', 'mainwp-sites-report' ) );
	}
	...
	public function action_save_post_meta( $post_id, $post, $update ) {
		...
		foreach ( $this->meta_boxes as $meta_box_key => $meta_box_data ) {
			...
			foreach ( $meta_box_data['fields'] as $group => $group_data ) {
				...
				l( $post_key );
				// do_action( 'iworks/mainwp-sites-report/postmeta/update', $post->ID, $option_name, $value, $key, $data );
			}
		}
	}
	...
	public function action_admin_enqueue_scripts_enqueue_assets() {
		$translation_array = array(
			...
			'l10n'            => array(
				'wp_media' => array(
					'title'  => esc_html__( 'Select or Upload Media', 'mainwp-sites-report' ),
					'button' => array(
						'text' => esc_html__( 'Use this Media', 'mainwp-sites-report' ),
					),
				),
			),
		);
		wp_localize_script(
			...
			'iworks_mainwp_sites_report',
			apply_filters(
				'iworks/mainwp-sites-report/wp_localize_script/admin',
				$translation_array
			)
		);
