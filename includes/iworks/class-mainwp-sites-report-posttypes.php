...
class iworks_wordpress_plugin_posttypes {
	...
	public function __construct() {
		...
		foreach ( glob( $posttypes_classes_dir . 'class*.php' ) as $filename_with_path ) {
			...
			if ( ! preg_match( '/^class-mainwp-sites-report-posttype-([a-z]+).php$/', $filename, $matches ) ) {
				continue;
			}
			$posttype_name = $matches[1];
			$filter        = sprintf(
				'mainwp-sites-report/load/posttype/%s',
				$posttype_name
			);
			if ( apply_filters( $filter, false ) ) {
				 ...
				 * Class Name
				 */
				$class_name = sprintf( 'iworks_mainwp_sites_report_posttype_%s', $posttype_name );
				/**
				 * Init Class Object
				 */
